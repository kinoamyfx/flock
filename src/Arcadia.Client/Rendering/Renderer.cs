...
/// <summary>
/// OpenGL 渲染器（清屏 + 2D Quad 绘制）
/// Why: 提供基础渲染能力，支持 2D 方块绘制（代表玩家/敌人/战利品）。
/// Context: MVP 阶段使用简单的 Quad 渲染，后续可升级为 Sprite Batching。
...
public sealed class Renderer : IDisposable
{
    ...
    public Renderer(IWindow window, GL gl)
    {
        _window = window;
        _gl = gl;

        Console.WriteLine("[Renderer] OpenGL initialized.");
        Console.WriteLine($"[Renderer] Vendor: {_gl.GetStringS(StringName.Vendor)}");
        Console.WriteLine($"[Renderer] Renderer: {_gl.GetStringS(StringName.Renderer)}");
        Console.WriteLine($"[Renderer] Version: {_gl.GetStringS(StringName.Version)}");
        ...
        Console.WriteLine($"[Renderer] Viewport set to {window.Size.X}x{window.Size.Y}");
    }

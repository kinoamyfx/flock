...
## Purpose
Define how the Gateway issues signed zone-handshake tokens (player identity binding, expiry, and key rotation), so the zone server can authenticate clients without trusting client-supplied identity fields.
